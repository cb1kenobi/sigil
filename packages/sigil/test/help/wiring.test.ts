...
/** What `main()` wrote to stdout, and the exit code it left behind. */
async function run(schema: Schema, argv: string[], settings?: Record<string, unknown>) {
	...
	try {
		await main({ argv, schema, settings: settings as never });
...
describe('Command.help', () => {
	...
	// it leaves through `main()`'s one error path like anything else
