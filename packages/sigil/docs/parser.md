...
```js
import { parse } from '@ttylabs/sigil';
...
| `helpExitCode`             | `0`     | Exit code `main()` sets after printing help                 |
...
```js
import main, { options } from '@ttylabs/sigil';
...
```js
import main, { command, options } from '@ttylabs/sigil';
...
if (state.help) {
	const { resolveHelp } = await import('sigil/help');
	process.stdout.write(`${await resolveHelp(state)}\n`);
	return;
}
```

`main()` does exactly that, and then sets `process.exitCode` from
...
`parse()` throws. `main()` catches — from the working directory check, from
...
`DEBUG=sigil:error` brings the stack back when you want it.
...
After an error is handled, `main()` resolves with `undefined`. It does not
...
throws — so calling `parse()` without `main()` gets the same error path — and
inside `main()`'s catch for everything else. Either way they fire once.
...
| throws          | Logged under `DEBUG=sigil:error` and skipped; the error it was handed stays in flight |
...
| `false`                 | `main()` rethrows; nothing is written, no code is set  |
| a function              | Replaces the handler; it owns output and the exit code |

A custom handler that throws rejects `main()`. That is a bug in the handler,
...
```js
import { errorHandler, renderError } from '@ttylabs/sigil/error-handler';
...
non-enumerably, and `main()` reads it back. Only an error thrown before there
