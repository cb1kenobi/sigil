...
The sigil toolchain. It will compile templates and stylesheets, resolve the
...
compiles against, [`@ttylabs/sigil`](../sigil), stays at zero dependencies.

There is no `sigil dev`. This is a resolver and a compiler, not a server.

## Status

Skeleton. The package exists, provides the `sigil` bin, and answers `--version`
...
package uses `main()` for its own argument parsing from the first commit
