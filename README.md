...
| ---------------------------------- | ---------------- | ---------------------------------------------------------------------------- |
| [`packages/sigil`](packages/sigil) | `@ttylabs/sigil` | The runtime an app is written against. **Zero dependencies, always.**        |
| [`packages/cli`](packages/cli)     | `@ttylabs/cli`   | The toolchain. Provides the `sigil` bin and may depend on whatever it needs. |
...
honest: an app that builds through `@ttylabs/cli` inlines the runtime and ships
no `node_modules` at all, and an app that skips the build still depends on
nothing.

See [`packages/sigil/README.md`](packages/sigil/README.md) for the API, and
[`packages/sigil/docs/parser.md`](packages/sigil/docs/parser.md) for the parser
reference.
...
`pnpm --filter @ttylabs/sigil test`.

## Demos

Small runnable examples, one idea each — see [`demos/`](demos). They import
`@ttylabs/sigil` by name, so they need a build first:
